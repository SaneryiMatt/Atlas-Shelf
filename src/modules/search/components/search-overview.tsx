...
import { Briefcase, Compass, Film, Search, SearchX } from "lucide-react";
...
function SearchGroupSection({ group }: { group: SearchResultGroup }) {
  ...
  return (
    <SectionCard title={`${group.title} · ${group.items.length}`} description={group.description}>
      {group.items.length ? (
...
export function SearchOverview({ query, totalCount, groups }: SearchOverviewProps) {
  return (
    <div className="space-y-8">
      <PageHeader
        eyebrow="全局搜索"
        title="在书籍、影视、旅行和投递里统一检索"
        description="搜索会覆盖标题、摘要、类型详情字段和标签。"
        actions={<GlobalSearchForm defaultValue={query} className="w-full max-w-xl" />}
      />

      {!query ? (
        <SectionCard title="开始搜索" description="输入关键词后，系统会统一返回四类结果。">
          <div className="rounded-3xl border border-dashed border-border bg-background/60 p-8 text-sm leading-7 text-muted-foreground">
            ...
            可以尝试搜索作品标题、作者、导演、平台、地点、公司、岗位或来源。
          </div>
        </SectionCard>
      ) : null}
        ...
        <section className="grid gap-4 md:grid-cols-3">
          <Card className="border-white/40 bg-white/80">
            <CardContent className="space-y-3 p-5">
              ...
              <p className="text-sm leading-6 text-muted-foreground">搜索会在四类内容中同时执行。</p>
            </CardContent>
          </Card>
          <Card className="border-white/40 bg-white/80">
            <CardContent className="space-y-3 p-5">
          ...
          <Card className="border-white/40 bg-white/80">
            <CardContent className="space-y-3 p-5">
              ...
              <p className="text-3xl font-semibold text-foreground">4 个模块</p>
              <p className="text-sm leading-6 text-muted-foreground">Books、Movies、Travels、Applications</p>
            </CardContent>
          </Card>
        </section>
      ) : null}
      ...
      {query && totalCount === 0 ? (
        <SectionCard title="没有找到结果" description="可以尝试更短的关键词，或改搜作者、地点、公司和岗位。">
          <div className="rounded-3xl border border-dashed border-border bg-background/60 p-8 text-sm leading-7 text-muted-foreground">
            <SearchX className="mb-4 size-5 text-primary" />
            “{query}” 当前没有命中任何书籍、影视、旅行或投递记录。
          </div>
        </SectionCard>
      ) : null}
